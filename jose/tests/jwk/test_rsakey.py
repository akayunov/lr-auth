from joserfc.jwk import RSAKey


def test_rsakey():
    rsakey1 = RSAKey.generate_key(2048, {'alg': 'RS256'})

    pem_file = """
    -----BEGIN PUBLIC KEY-----
    
    -----END PUBLIC KEY-----
    """
    rsakey2 = RSAKey.import_key(pem_file)

    assert rsakey1.as_dict()['e'] == 'AQAB'
    assert rsakey1.as_dict()['kty'] == 'RSA'
    assert rsakey2.private_key is None

    assert rsakey1.alg == 'RS256'